...
class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/', device='auto'):
    self.gt_pose = None
  ...
  def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
    ...
    self.pts = torch.tensor(np.asarray(pcd.points), dtype=torch.float32, device='cpu')
    self.normals = F.normalize(torch.tensor(np.asarray(pcd.normals), dtype=torch.float32, device='cpu'), dim=-1)
    ...
    if symmetry_tfs is None:
      self.symmetry_tfs = torch.eye(4, device=self.device).float()[None]
    else:
  ...
  def get_tf_to_centered_mesh(self):
    tf_to_center = torch.eye(4, dtype=torch.float, device='cpu')
    tf_to_center[:3,3] = -torch.as_tensor(self.model_center, device='cpu', dtype=torch.float)
    return tf_to_center


  def to_device(self, s='cpu:0'):
    self.device = get_torch_device(s)
    for k in self.__dict__:
      self.__dict__[k] = self.__dict__[k]
      if torch.is_tensor(self.__dict__[k]) or isinstance(self.__dict__[k], nn.Module):
        logging.info(f"Moving {k} to device {self.device}")
        self.__dict__[k] = self.__dict__[k].to(self.device)
    for k in self.mesh_tensors:
      logging.info(f"Moving {k} to device {self.device}")
      self.mesh_tensors[k] = self.mesh_tensors[k].to(self.device)
    if self.refiner is not None:
      self.refiner.model.to(self.device)
    if self.scorer is not None:
      self.scorer.model.to(self.device)
    if self.glctx is not None:
      self.glctx = dr.RasterizeCudaContext(str(self.device)) if self.device.type == 'cuda' else dr.RasterizeGLContext()
  ...
  def make_rotation_grid(self, min_n_views=40, inplane_step=60):
    ...
    self.rot_grid = torch.as_tensor(rot_grid, device='cpu', dtype=torch.float)
  ...
  def generate_random_pose_hypo(self, K, rgb, depth, mask, scene_pts=None):
    ...
    ob_in_cams[:,:3,3] = torch.tensor(center, device='cpu', dtype=torch.float).reshape(1,3)
    return ob_in_cams
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    depth = erode_depth(depth, radius=2, device='cpu')
    depth = bilateral_filter_depth(depth, radius=2, device='cpu')
    ...
    poses = torch.as_tensor(poses, device='cpu', dtype=torch.float)
    poses[:,:3,3] = torch.as_tensor(center.reshape(1,3), device='cpu')
  ...
  def compute_add_err_to_gt_pose(self, poses):
    ...
    '''
    return -torch.ones(len(poses), device='cpu', dtype=torch.float)
  ...
  def track_one(self, rgb, depth, K, iteration, extra={}):
    ...
    depth = torch.as_tensor(depth, device='cpu', dtype=torch.float)
    depth = erode_depth(depth, radius=2, device='cpu')
    depth = bilateral_filter_depth(depth, radius=2, device='cpu')
    logging.info("depth processing done")

    xyz_map = depth2xyzmap_batch(depth[None], torch.as_tensor(K, dtype=torch.float, device='cpu')[None], zfar=np.inf)[0]
